...
with open('MDL_Variety_show.csv', mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    
    writer.writerow(['Web Series Name', 'Original Language - Year of Release', 'Episodes', 'Overview', 'MDL Rating'])

    max_pages = 250 

    for page_number in range(1, max_pages + 1):
        try:
            url = f'https://mydramalist.com/shows/variety?page={page_number}'
            ...
            for movie in movies:
                try:
                    ...
                    print(title, year, episodes, MDL_rating, overview)
                    
                    
                    writer.writerow([title, year, episodes, overview, MDL_rating])
